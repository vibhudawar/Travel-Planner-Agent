...
def _price_for(model: str) -> dict:
    if model in PRICES:
        return PRICES[model]
    # Providers report dated variants (e.g. gpt-4o-2024-08-06) — prefix match.
...
def compute_cost(usage_metadata: dict) -> dict:
    """Turn LangChain usage_metadata ({model: {input_tokens, output_tokens}}) into cost."""
    by_model = {}
    total = 0.0
    for model, usage in (usage_metadata or {}).items():
        ...
        by_model[model] = {"input_tokens": inp, "output_tokens": out, "cost_usd": round(cost, 6)}
        total += cost
    return {
